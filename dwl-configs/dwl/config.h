...
static const float rootcolor[] = COLOR(0x121317ff);
...
// static const float bordercolor[] = COLOR(0x1a1a2200);
// static const float focuscolor[] = COLOR(0x7094c000); // making gap by border

/* Autostart */

static const char *const autostart[] = {
    "sh", "-c", "~/.scripts/wayland/dwl/autostart.sh", NULL};
...
#define TAGKEYS(KEY, SKEY, TAG)                                                \
  {WLR_MODIFIER_CTRL, KEY, view, {.ui = 1 << TAG}},                            \
      {WLR_MODIFIER_CTRL | WLR_MODIFIER_SHIFT, SKEY, tag, {.ui = 1 << TAG}},   \
      {WLR_MODIFIER_CTRL | WLR_MODIFIER_ALT,                                   \
       KEY,                                                                    \
       toggleview,                                                             \
       {.ui = 1 << TAG}},                                                      \
  {                                                                            \
    WLR_MODIFIER_CTRL | WLR_MODIFIER_ALT | WLR_MODIFIER_SHIFT, SKEY,           \
        toggletag, {                                                           \
      .ui = 1 << TAG                                                           \
    }                                                                          \
  }
...
static const Key keys[] = {
    ...
    {0, XKB_KEY_XF86AudioLowerVolume, spawn,
     SHCMD("~/.scripts/wayland/vol.sh down someblocks RTMIN+6")},
    {0, XKB_KEY_XF86AudioMute, spawn,
     SHCMD("pw-volume mute toggle && pkill -RTMIN+6 someblocks")},
